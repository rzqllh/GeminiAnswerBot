  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorMessage.includes('API key not valid')) {
        ...
        errorMessage =
          'Batas penggunaan API tercapai. Silakan periksa kuota Anda di Google Cloud Console.';
      } else if (response.status === 400 && errorMessage.includes('Unsupported content')) {
        errorMessage =
          'Konten tidak didukung oleh model AI. Coba berikan teks yang berbeda atau lebih singkat.';
      }
      ...
      for (const line of lines) {
        try {
          ...
          if (data.candidates === undefined && data.promptFeedback) {
            ...
            if (blockReason) {
              throw new Error(
                `Permintaan diblokir oleh AI karena alasan keamanan: ${blockReason}.`
              );
            }
          }
    ...
    fetch(apiUrl, {
      ...
      .then((result) => {
        ...
        if (text.toUpperCase() === 'OK') {
          ...
          sendResponse({
            success: false,
            error: `Tes gagal. AI merespons dengan tidak terduga: "${text}"`,
          });
        }
      })
