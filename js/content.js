...
if (typeof window.geminiAnswerBotContentScriptLoaded === 'undefined') {
  ...
  function initializeMarker() {
    if (typeof Mark !== 'undefined' && !markerInstance) {
      ...
      console.error(
        'Content Script: Mark.js library (window.Mark) not found. Highlighting may not work.'
      );
    }
  }
  ...
  function _tryLevel1Extraction() {
    ...
    for (const selector of quizContainerSelectors) {
      ...
      if (
        ...
        let quizQuestion = '';
        const questionCandidates = container.querySelectorAll(
          'h3, h4, p:first-of-type, div.w3-large, div[class*="question"]'
        );
        for (const qEl of questionCandidates) {
          let qText = qEl.textContent
            .trim()
            .replace(/^Question\s+\d+\s+of\s+\d+\s*[:-]?\s*/i, '')
            .trim();
          if (
            qText.length > 10 &&
            !qText.toLowerCase().includes('quiz') &&
            !qText.toLowerCase().includes('html quiz')
          ) {
            quizQuestion = qText;
            break;
          }
        ...
        const quizOptions = [];
        const rawOptionElements = container.querySelectorAll(
          'input[type="radio"] + label, input[type="checkbox"] + label, div[class*="option"], div.w3-code, div.w3-example, li, p'
        );
        ...
        rawOptionElements.forEach((el) => {
          ...
          if (codeOrPreEl) {
            ...
            if (
              (innerText.startsWith('<') && innerText.endsWith('>') && innerText.length < 50) ||
              (innerHTML.includes('&lt;') && innerHTML.includes('&gt;') && innerHTML.length < 100)
            ) {
          ...
          if (
            ...
            !optionContent.toLowerCase().includes('next') &&
            !optionContent.toLowerCase().includes('submit') &&
            !optionContent.toLowerCase().includes('previous') &&
            !seenOptions.has(optionContent.toLowerCase())
          ) {
  ...
  function _tryLevel2Extraction() {
    ...
    clone.childNodes.forEach((node) => {
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (['script', 'style', 'nav', 'header', 'footer', 'aside', 'iframe'].includes(tagName))
          return;
        ...
        if (
          text.length > 1 &&
          text.length < 1000 &&
          !/advertisement|cookie|login|sign up|next|previous|submit|test your skills/i.test(text)
        ) {
          content += text + '\n\n';
        }
  ...
  function _tryLevel3Fallback() {
    ...
    bodyCloneFinal
      .querySelectorAll(
        'script, style, noscript, iframe, .hidden, [style*="display:none"], [aria-hidden="true"]'
      )
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'get_page_content') {
      ...
      sendResponse({
        content: pageContent,
        source: selectedText.length > 20 ? 'selection' : 'auto',
      });
      return true;
