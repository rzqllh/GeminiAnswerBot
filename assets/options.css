...
:root {
  ...
  --slider-track-bg: rgba(120, 120, 128, 0.16);
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --slider-track-bg: rgba(120, 120, 128, 0.36);
...
/* === REFINED SLIDER STYLES (Apple HIG Inspired) === */
.slider-container {
  position: relative;
  display: flex;
  align-items: center;
...
.slider-container .slider-value-display.tooltip-visible {
  opacity: 1;
}

.slider-container .slider-value-display::after {
  content: '';
  position: absolute;
  top: 100%;
  left: 50%;
...
#history-list-container { padding: 0; }
.loading-state, .empty-state {
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    text-align: center; color: var(--secondary-text); padding: 60px 20px;
}
.spinner { width: 28px; height: 28px; border-radius: 50%; margin-bottom: 16px; border: 4px solid var(--border-color); border-top-color: var(--accent-blue); animation: spin 1s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.empty-state svg { margin-bottom: 16px; color: var(--secondary-text); opacity: 0.5; }
.empty-state h3 { margin: 0 0 8px 0; font-size: 18px; color: var(--primary-text); }
...
.history-card:hover { box-shadow: 0 6px 18px rgba(0,0,0,0.08); transform: translateY(-2px); }
@media (prefers-color-scheme: dark) {
    .history-card:hover { box-shadow: 0 6px 18px rgba(0,0,0,0.2); }
}

.history-card-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 20px; border-bottom: 1px solid var(--border-color); }
.history-card-title { font-weight: 600; text-decoration: none; color: var(--primary-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 400px; font-size: 15px; }
.history-card-title:hover { color: var(--accent-blue); }
.history-card-date { font-size: 13px; color: var(--secondary-text); flex-shrink: 0; margin-left: 16px; }
.history-card-body { padding: 20px; display: flex; flex-direction: column; gap: 20px; }
.history-section { background-color: var(--bg-color); border-radius: 10px; padding: 16px; }
.history-section-title { font-size: 13px; font-weight: 600; color: var(--secondary-text); margin: 0 0 12px 0; text-transform: uppercase; letter-spacing: 0.5px; }
.history-question-text { font-weight: 500; margin: 0 0 12px 0; }
.history-section ul { list-style-type: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 8px; }
.history-section li { padding-left: 20px; position: relative; }
.history-section li::before { content: '•'; position: absolute; left: 0; color: var(--accent-blue); }
.history-ai-response { margin: 0; line-height: 1.6; }
.history-ai-response strong { font-weight: 600; color: var(--primary-text); }
.confidence-tag { padding: 3px 8px; border-radius: 6px; font-size: 12px; font-weight: 600; color: #1d1d1f; }
.confidence-high { background-color: var(--accent-green); }
.confidence-medium { background-color: var(--accent-yellow); }
.confidence-low { background-color: var(--accent-red); color: #fff; }
...
@media (prefers-color-scheme: dark) { .custom-toast { background-color: var(--toast-bg-dark); border-color: var(--toast-border-dark); } }
